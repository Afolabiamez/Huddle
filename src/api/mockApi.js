...
// Exists so AppContext can call `api.fetchMessages(channelId)` the same way
// regardless of whether it's talking to the mock or the real backend, even
// though the mock already embeds messages on the channel object.
export async function fetchMessages(channelId) {
  const channel = channels.find((c) => c.id === channelId);
  return delay(channel?.messages ?? []);
}
